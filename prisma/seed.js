import { PrismaClient, UserRole, UserStatus, CertificationStatus, AvailabilityStatus, PlantHealthStatus, } from "@prisma/client";
...
async function main() {
    ...
    const vendorUsers = await Promise.all(Array.from({ length: 10 }).map((_, index) => prisma.user.create({
        data: {
            name: `Vendor ${index + 1}`,
    ...
    const vendorProfiles = await Promise.all(vendorUsers.map((user, index) => prisma.vendorProfile.create({
        data: {
            userId: user.id,
            farmName: `Green Plot ${index + 1}`,
            certificationStatus: index % 2 === 0
    ...
    const produceSeed = Array.from({ length: 100 }).map((_, index) => ({
        ...
        category: index % 3 === 0 ? "Vegetables" : index % 3 === 1 ? "Seeds" : "Tools",
        certificationStatus: index % 2 === 0
            ? CertificationStatus.VERIFIED
            : CertificationStatus.PENDING,
