...
export class VendorService {
  static async listVendors(userRole: string) {
  ...
  static async createCertification(
    ...
    if (requester.role === UserRole.VENDOR) {
      ...
      if (targetVendorId !== ownVendor.id) {
        throw new AppError(
          403,
          "You can only submit certifications for yourself",
        );
      }
    }
  ...
  static async reviewCertification(
    ...
    const updatedCert = await vendorQuery.updateCertificationStatus(
      certId,
      status,
    );
    if (status === "VERIFIED") {
      await vendorQuery.updateVendorCertificationStatus(
        cert.vendorId,
        "VERIFIED",
      );
    }
